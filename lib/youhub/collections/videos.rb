...
module Youhub
  module Collections
    ...
    #
    # Resources with videos are: {Youhub::Models::Channel channels} and
    # {Youhub::Models::Account accounts}.
    class Videos < Base
      ...
      def eager_load_items_from(items)
        if included_relationships.any?
          associations = %i[claim category]
          included_relationships.append(:snippet).uniq! if (included_relationships & associations).any?
      ...
      def use_list_endpoint?
        @where_params ||= {}
        @parent.nil? && (@where_params.keys & %i[id chart]).any?
      end
    end
  end
end

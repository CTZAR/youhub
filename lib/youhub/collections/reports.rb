...
module Youhub
  module Collections
    ...
    class Reports < Base
      ...
      # @see https://developers.google.com/youtube/analyouhubics/v1/dimsmets/dims#Traffic_Source_Dimensions
      ...
      TRAFFIC_SOURCES = {
        ...
        channel:       'YOUHUB_CHANNEL',
        other_page:    'YOUHUB_OTHER_PAGE',
        search:        'YOUHUB_SEARCH',
        google:        'GOOGLE_SEARCH',
        notification:  'NOTIFICATION',
        playlist_page: 'YOUHUB_PLAYLIST_PAGE',
        campaign_card: 'CAMPAIGN_CARD',
        end_screen:    'END_SCREEN',
        info_card:     'INFO_CARD'
      }.freeze

      # @see https://developers.google.com/youtube/analyouhubics/v1/dimsmets/dims#Playback_Location_Dimensions
      PLAYBACK_LOCATIONS = {
        channel:      'CHANNEL',
        watch:        'WATCH',
        embedded:     'EMBEDDED',
      ...
      # @see https://developers.google.com/youtube/analyouhubics/v1/dimsmets/dims#Playback_Detail_Dimensions
      SUBSCRIBED_STATUSES = {
        subscribed:   'SUBSCRIBED',
        unsubscribed: 'UNSUBSCRIBED'
      }.freeze

      # @see https://developers.google.com/youtube/analyouhubics/v1/dimsmets/dims#youtubeProduct
      YOUTUBE_PRODUCTS = {
        core:    'CORE',
        gaming:  'GAMING',
        kids:    'KIDS',
      ...
      # @see https://developers.google.com/youtube/analyouhubics/v1/dimsmets/dims#Device_Dimensions
      DEVICE_TYPES = {
        desktop:          'DESKTOP',
      ...
      def within(days_range, country, state, dimension, videos, historical, max_retries = 3)
        ...
        if dimension == :gender_age_group # array of array
          ...
          hash = hash.inject(@metrics.transform_values { |_v| {} }) do |result, hash|
            result.deep_merge hash
          end
          if dimension == :month
            hash = hash.transform_values { |h| h.sort_by { |range, _v| range.first }.to_h }
          elsif dimension == :week
            hash = hash.transform_values do |h|
              h.select { |range, _v| range.last.wday == days_range.last.wday }
               .sort_by { |range, _v| range.first }.to_h
            end
          elsif dimension == :day
            hash = hash.transform_values { |h| h.sort_by { |day, _v| day }.to_h }
          elsif dimension.in? %i[traffic_source country state playback_location device_type operating_system subscribed_status]
            hash = hash.transform_values { |h| h.sort_by { |_range, v| -v }.to_h }
          end
          @metrics.one? || @metrics.keys == %i[estimated_revenue estimated_minutes_watched] ? hash[@metrics.keys.first] : hash
        end
      ...
      def reports_params
        @parent.reports_params.tap do |params|
          ...
          params['maxResults'] = 50 if @dimension.in? %i[playlist video]
          params['maxResults'] = 25 if @dimension.in? %i[embedded_player_location related_video search_term referrer]
          if @dimension.in? %i[video playlist embedded_player_location related_video search_term referrer]
            params['sort'] = if @metrics.keys == %i[estimated_revenue estimated_minutes_watched]
                               '-estimatedRevenue'
                             else
                               "-#{@metrics.keys.join(',').to_s.camelize(:lower)}"
                             end
          end
          ...
          params[:filters] = ((params[:filters] || '').split(';') + ['insightTrafficSourceType==YOUHUB_SEARCH']).compact.uniq.join(';') if @dimension == :search_term

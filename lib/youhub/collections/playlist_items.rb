  ...
  module Collections
    class PlaylistItems < Resources
      def insert(attributes = {}, options = {})
        ...
        ignorable_errors = e.reasons & %w[videoNotFound forbidden]
        raise e unless options[:ignore_errors] && ignorable_errors.any?
      end
      ...
      def insert_parts
        { snippet: { keys: %i[playlist_id resource_id position] } }
      end
    end
  end
end

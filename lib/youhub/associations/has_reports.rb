...
module Youhub
  module Associations
    # @private
    # Provides methods to access the analyouhubics reports of a resource.
    module HasReports
      # @!macro [new] reports
      ...
      #   @return [Hash<Youhub::Video, $2>] if grouped by related video, the
      ...
      #     # => {#<Youhub::Video @id=…> => 33.0, #<Youhub::Video @id=…> => 12.0, …}
      ...
      #     # => {"Google Search" => 33.0, "youhubimg.com" => 12.0, …}
      #   @return [Hash<String, $2>] if grouped by search term, the
      #     $1 for each search term that led viewers to the content.
      ...
      #   @return [Hash<Youhub::Video, $2>] if grouped by video, the
      #     $1 for each video.
      ...
      #     # => {#<Youhub::Video @id=…> => 33.0, #<Youhub::Video @id=…> => 12.0, …}
      #   @return [Hash<Youhub::Playlist, $2>] if grouped by playlist, the
      #     $1 for each playlist.
      #   @example Get yesterday’s $1 by playlist:
      #     resource.$1 since: 1.day.ago, until: 1.day.ago, by: :playlist
      #     # => {#<Youhub::Playlist @id=…> => 33.0, #<Youhub::Playlist @id=…> => 12.0, …}
      #   @macro report_with_video_dimensions
      ...
      def define_reports_method(metric, type)
        ...
        unless defined?(reports)
          define_method :reports do |options = {}|
            from = options[:since] || options[:from] || (options[:by].in?(%i[day week month]) ? 5.days.ago : '2005-02-01')
            ...
            reports = Collections::Reports.of(self).tap do |reports|
              reports.metrics = self.class.instance_variable_get(:@metrics).select { |k, _v| k.in? only }
            end
      ...
      def define_metric_method(metric)
        define_method metric do |options = {}|
          ...
          lookup_class = case options[:by]
                         when :video, :related_video then Youhub::Collections::Videos
                         when :playlist then Youhub::Collections::Playlists
          end
          if lookup_class

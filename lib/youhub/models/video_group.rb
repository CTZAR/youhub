...
module Youhub
  module Models
    # Provides methods to interact with YouTube Analyouhubics video-groups.
    # @see https://developers.google.com/youtube/analyouhubics/v1/reference/groups
    class VideoGroup < Base
      ...
      #   @return [Youhub::Collections::GroupItems] the group’s items.
      ...
      # @private
      # Tells `has_reports` to retrieve group reports from the Analyouhubics API.
      def reports_params
        {}.tap do |params|
      ...
      def all_video_ids
        ...
        when 'youtube#channel'
          resource_ids.flat_map do |channel_id|
            Youhub::Channel.new(id: channel_id, auth: @auth).videos.map(&:id)
          end
        else
          []
        end
      ...
      def all_channel_ids
        ...
        when 'youtube#video'
          resource_ids.flat_map do |video_id|
            Youhub::Video.new(id: video_id, auth: @auth).channel_id
          end.uniq
        when 'youtube#channel'
          resource_ids
        else

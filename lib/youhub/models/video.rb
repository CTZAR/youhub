...
module Youhub
  module Models
    ...
    class Video < Resource
      ...
      def age_restricted?
        content_detail.youtube_rating == 'youhubAgeRestricted'
      end
      ...
      #   @return [Integer] the size of the uploaded file (in byouhubes).
      ...
      # @raise [Youhub::Errors::Unauthorized] if {Resource#auth auth} is not an
      #   authenticated Youhub::Account.
      def liked?
        rating.rating == :like
      end
      ...
      #   @return [Youhub::Collections::Comments] the video’s comments.
      has_many :comment_threads

      # @!attribute [r] annotations
      #   @return [Youhub::Collections::Annotations] the video’s annotations.
      has_many :annotations

      has_many :resumable_sessions

      # @!attribute [r] channel
      #   @return [Youhub::Models::Claim, nil] the first claim on the video by
      ...
      # @raise [Youhub::Errors::RequestError] if path_or_url is not a valid path
      #   or URL.
      ...
      # @raise [Youhub::Errors::Unauthorized] if {Resource#auth auth} is not an
      #   authenticated Youhub::Account with permissions to delete the video.
      def delete(_options = {})
        do_delete { @id = nil }
        !exists?
      end
      ...
      # @raise [Youhub::Errors::Unauthorized] if {Resource#auth auth} is not an
      #   authenticated Youhub::Account with permissions to update the video.
      ...
      #   Cannot have more than 5000 byouhubes. Can include the characters
      #   < and >, which are replaced to ‹ › in order to be accepted by YouTube.
      ...
      def initialize(options = {})
        ...
        @file_detail = FileDetail.new data: options[:file_details] if options[:file_details]
        if options[:live_streaming_details]
          @live_streaming_detail = LiveStreamingDetail.new data: options[:live_streaming_details]
        end
        @video_category = VideoCategory.new data: options[:video_category] if options[:video_category]
        @player = Player.new data: options[:player] if options[:player]
        @claim = options[:claim] if options[:claim]
      end
      ...
      # @private
      # Tells `has_reports` to retrieve the reports from YouTube Analyouhubics API
      # either as a Channel or as a Content Owner.
      # @see https://developers.google.com/youtube/analyouhubics/v1/reports
      def reports_params
        {}.tap do |params|
          params[:ids] = if auth.owner_name
                           "contentOwner==#{auth.owner_name}"
                         else
                           "channel==#{channel_id}"
                         end
      ...
      def update_parts
        snippet_keys = %i[title description tags category_id]
        snippet = { keys: snippet_keys, sanitize_brackets: true }
        status_keys = %i[privacy_status embeddable license
      ...
      def build_update_body_part(name, part, attributes = {})
        {}.tap do |body_part|
          ...
          if (body_part[:publishAt] || 1.day.from_now) < Time.now || body_part[:privacyStatus].in?(%w[public unlisted])

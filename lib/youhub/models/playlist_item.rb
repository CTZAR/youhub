...
module Youhub
  module Models
    ...
    class PlaylistItem < Resource
      ...
      #   authenticated Youhub::Account with permissions to delete the item.
      def delete(_options = {})
        do_delete { @id = nil }
        !exists?
      end
      ...
      # @raise [Youhub::Errors::Unauthorized] if {Resource#auth auth} is not an
      #   authenticated Youhub::Account with permissions to update the item.
      ...
      # @return [Youhub::Models::Video] the video referred by the item.
      def video
      ...
      def update_parts
        keys = %i[position playlist_id resource_id]

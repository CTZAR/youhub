...
module Youhub
  module Models
    ...
    #   url = Youhub::URL.new 'youtu.be/kawaiiguy'
    #   url.id # => 'UC4lU5YG9QDgs0X2jdnt7cdQ'
    #   url.resource # => #<Youhub::Channel @id=UC4lU5YG9QDgs0X2jdnt7cdQ>
    class URL
      ...
      # @return [<Youhub::Channel>] the resource associated with the URL
      def resource(options = {})
        @resource ||= case kind
                      when :channel then Youhub::Channel
                      when :video then Youhub::Video
                      when :playlist then Youhub::Playlist
                      else raise Youhub::Errors::NoItems
      ...
      def fetch_id
        ...
        regex = /<meta itemprop="channelId" content="(?<id>UC[a-zA-Z0-9_-]{22})">/
        if data = response.body.match(regex)
          data[:id]
        else
          raise Youhub::Errors::NoItems
        end
      end
    end
  end

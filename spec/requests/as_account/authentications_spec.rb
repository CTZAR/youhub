...
describe Youhub::Account, :device_app do
  subject(:account) { Youhub::Account.new attrs }

  describe '#refresh' do
    context 'given a valid refresh token' do
      let(:attrs) { { refresh_token: ENV['YOUHUB_TEST_DEVICE_REFRESH_TOKEN'] } }
  ...
  describe '#authentication' do
    context 'given a refresh token' do
      ...
      context 'that is valid' do
        let(:refresh_token) { ENV['YOUHUB_TEST_DEVICE_REFRESH_TOKEN'] }
        it { expect(account.authentication).to be_a Youhub::Authentication }
        it { expect(account.refresh_token).to eq refresh_token }
      end

      context 'that is invalid' do
        let(:refresh_token) { '--not-a-valid-refresh-token--' }
        it { expect { account.authentication }.to raise_error Youhub::Errors::Unauthorized }
      end
    end
    ...
    context 'given an access token' do
      ...
      context 'that is valid' do
        ...
        context 'that has expired' do
          ...
          context 'and no refresh token' do
            it { expect { account.authentication }.to raise_error Youhub::Errors::MissingAuth }
          end
          ...
          context 'and a valid refresh token' do
            before { attrs[:refresh_token] = ENV['YOUHUB_TEST_DEVICE_REFRESH_TOKEN'] }
            it { expect(account.authentication).to be_a Youhub::Authentication }
          end
        end
      end
      ...
      context 'that is invalid' do
        ...
        context 'and no refresh token' do
          it { expect { account.channel }.to raise_error Youhub::Errors::Unauthorized }
        end

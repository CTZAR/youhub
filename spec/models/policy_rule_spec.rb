...
require 'spec_helper'
require 'youhub/models/policy_rule'

describe Youhub::PolicyRule do
  subject(:policy_rule) { Youhub::PolicyRule.new data: data }
  ...
  describe '#included_territories' do
    context 'given fetching a policy rule returns included territories' do
      let(:data) { { 'conditions' => { 'requiredTerritories' => { 'type' => 'include', 'territories' => %w[US CA] } } } }
      let(:data) { { 'conditions' => { 'requiredTerritories' => { 'type' => 'include', 'territories' => %w[US CA] } } } }
  ...
  describe '#excluded_territories' do
    context 'given fetching a policy rule returns excluded territories' do
      let(:data) { { 'conditions' => { 'requiredTerritories' => { 'type' => 'exclude', 'territories' => %w[US CA] } } } }

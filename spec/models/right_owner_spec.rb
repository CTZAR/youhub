...
require 'spec_helper'
require 'youhub/models/right_owner'

describe Youhub::RightOwner do
  subject(:right_owner) { Youhub::RightOwner.new data: data }
  let(:data) { {} }
  ...
  describe '#included_territories' do
    context 'given fetching an owner returns included territories' do
      let(:data) { { 'type' => 'include', 'territories' => %w[US CA] } }
  ...
  describe '#excluded_territories' do
    context 'given fetching an owner returns excluded territories' do
      let(:data) { { 'type' => 'exclude', 'territories' => %w[US CA] } }
